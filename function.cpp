...
// 各ファイバーそれぞれで力を計算する
// 収縮力(動径方向)
Eigen::Array<double, 1, 2> calc_contraction_force_rf(const Eigen::Array<double, 1, 2>& p1, const Eigen::Array<double, 1, 2>& p2, double thickness) {
  // 位置座標単位ベクトル化用
...
// 収縮力(外周方向)
Eigen::Array<double, 1, 2> calc_contraction_force_of(const Eigen::Array<double, 1, 2>& p1, const Eigen::Array<double, 1, 2>& p2, double thickness) {
  double fiber_length = (p1-p2).matrix().norm();
...
// 伸展力を計算する 動径方向のみ
Eigen::Array<double, 1, 2> calc_extension_force(const Eigen::Array<double, 1, 2>& p1, const Eigen::Array<double, 1, 2>& p2, double thickness) {
  double fiber_length = (p1-p2).matrix().norm();
...
double calc_thickness_variation_rf(const Fiber& f) {
  ...
  if (fiber.length() > init_rf_length) h_i = calc_restoring_force_rf(f); // 自然長より長いなら復元力が働く
